 ...
 * The return value comes packaged as a triplet, because someone had the bright
 * fucking idea of superclassing something with read() for that whole 
 * polymorphism thing that we NEVER USE. W H Y 
 */
double spacehauc_i2c::TSL2561::read() {
        Triplet<double> luminosity_val;
        ...
        // Convert the data
        return luminosity_val.X(((data[1]) * 256 + data[0]));
}
 ...
 */
spacehauc_i2c::Triplet<double> spacehauc_i2c::LSM303_Magnetometer::read() {
        uint8_t buffer[6] = {0};
...
spacehauc_i2c::Triplet<double> spacehauc_i2c::L3GD20::read() {
        uint8_t buffer[6] = {0};
